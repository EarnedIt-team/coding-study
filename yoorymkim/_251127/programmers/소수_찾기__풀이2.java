...
public class 소수_찾기__풀이2 {
    ...
    class Solution {
        ...
        private void dfs(String current, int depth) {
            // current : 만들어받은 문자열
            if (current.length() > 0) { // 만들어짐?
                int num = Integer.parseInt(current);
                if (isPrime(num)) set.add(num); // 소수만 저장
            }

            if (depth == arr.length) return; // 더 이상 뽑을 문자가 없다

            for (int i = 0; i < arr.length; i++) {
                if (!visited[i]) { // if 아직 쓰지 않은 숫자원소
                    visited[i] = true; // 사용
